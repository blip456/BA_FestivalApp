...
namespace Ypsilon2.model
{
    public class Festival
    {
        ...
        public static ObservableCollection<string> aantalDagen()
        {
            ObservableCollection<string> lstDagen = new ObservableCollection<string>();
            Festival festival = GetFestivals();
            TimeSpan timespan = festival.EndDate - festival.StartDate;
            for (int i = 1; i <= timespan.Days+1; i++)
            {
                lstDagen.Add("Dag " + i);
            }
